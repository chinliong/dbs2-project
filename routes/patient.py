...
def patient_dashboard():
    if 'is_staff' in session and session['is_staff'] == 0:
        ...
        # Fetch patient details
        patient = db['Patients'].find_one({"user_id": ObjectId(session['user_id'])})

        # Fetch appointments for patient
        if patient:
            appointments = list(db['Appointments'].find({"patient_id": patient['_id']}).sort([("ApptDate", -1), ("ApptTime", -1)]))
        else:
            appointments = []
...
def update_account():
    ...
    if request.method == 'POST':
        ...
        # Fetch the existing user and check for duplicate email
        existing_user = db['Users'].find_one({"email": email, "_id": {"$ne": user_id}})
        if existing_user:
            flash('Email is already in use by another account.')
            return redirect(url_for('patient.update_account'))

        # Hash the new password if provided, else retain the old one
        if password.strip():
            hashed_password = generate_password_hash(password, method='pbkdf2:sha256')
        else:
            hashed_password = db['Users'].find_one({"_id": user_id})['password']
    ...
    # Fetch the current user's data, including patient details
...
def book_appointment():
    ...
    # Set the allowable date range for booking
    today = datetime.now().date()
    one_week_later = today + timedelta(days=7)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Ensure appointments are booked in 30-minute intervals
            if appt_time_obj.minute not in [0, 30]:
                flash('Appointments must be booked at 30-minute intervals.')
                return redirect(url_for('patient.book_appointment'))
        except ValueError:
        ...
        if not (today <= appt_date_obj <= one_week_later):
            flash('Appointments can only be booked within the next 7 days.')
            return redirect(url_for('patient.book_appointment'))

        db = get_db_connection()

        # Fetch the patient document to get their patient ID
        patient = db['Patients'].find_one({"user_id": ObjectId(session['user_id'])})
        if not patient:
            flash('Patient record not found. Please contact support.')
            return redirect(url_for('patient.patient_dashboard'))
        ...
        if db['Appointments'].find_one({"ApptDate": appt_date_obj, "ApptTime": appt_time_obj}):
            flash('This appointment slot is already taken. Please choose another time.')
            return redirect(url_for('patient.book_appointment'))
